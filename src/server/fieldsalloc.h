...
*/
  ...
  public:
    ...
    explicit
    FieldsAlloc(std::size_t size)
        : pool_(&detail::StaticPool::construct(size))
    {
    }

    FieldsAlloc(FieldsAlloc const& other)
        : pool_(&other.pool_->share())
    {
    }

    template<class U>
    FieldsAlloc(FieldsAlloc<U> const& other)
        : pool_(&other.pool_->share())
    {
    }
    ...
    bool
    operator==(
        FieldsAlloc const& lhs,
        FieldsAlloc<U> const& rhs)
    {

...
beast::string_view spt::server::mime_type(beast::string_view path)
{
  ...
  if(iequals(ext, ".pptx")) return "application/vnd.openxmlformats-officedocument.presentationml.presentation";
...
std::string spt::server::path_cat(  beast::string_view base, beast::string_view path )
{
  if(base.empty())
